...
void ACatCharacter::OnInteract_Implementation()
{
	...
	else
	{
		...
		{
			if (PlayerCharacter->HasItemOnHands(Quest.ItemID) && PlayerCharacter->GetItemInInventory(Quest.ItemID).Quantity == Quest.quantityNeeded)
			{
				PlayerCharacter->RemoveItem(Quest.ItemID, true, Quest.quantityNeeded);
